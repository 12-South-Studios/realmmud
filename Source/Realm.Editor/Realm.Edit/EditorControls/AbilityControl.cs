    ...
    public partial class AbilityControl : BaseEditorControl
    {
        ...
        public override bool SaveImpl()
        {
            try
            {
                ...
                {
                    var systemString = new SystemString
                    {
                        StringType = StringTypes.DisplayName,
                        Value = txtDisplayName.Text
                    };
                    dbContext.SystemStrings.Add(systemString);
                    ability.DisplayName = systemString;
                }
                else
                    ability.DisplayName.Value = txtDisplayName.Text;

                if (ability.DisplayDescription == null)
                {
                    var systemString = new SystemString
                    {
                        StringType = StringTypes.DisplayDescription,
                        Value = txtDisplayDescription.Text
                    };
                    dbContext.SystemStrings.Add(systemString);
                    ability.DisplayDescription = systemString;
                }
                else
                    ability.DisplayDescription.Value = txtDisplayDescription.Text;
        ...
        public override bool IsSaveValid(bool aGiveFeedback)
        {
            ...
            hasErrors = txtDisplayName.ValidateField(() => txtDisplayName.Text.Length == 0, "Missing Display Name");
            hasErrors = txtSystemName.ValidateField(() => txtSystemName.Text.Length == 0, "Missing System Name");
            hasErrors = txtDisplayDescription.ValidateField(() => txtDisplayDescription.Text.Length == 0,
                "Missing Display Description");
            if (chkTerrainRequired.Checked)
                hasErrors = linkTerrain.ValidateField(() => string.IsNullOrEmpty(linkTerrain.Text), "Missing Terrain");
            hasErrors = chkAutoAttack.ValidateField(() => chkAutoAttack.Checked && chkPassive.Checked,
                "Cannot be Auto-Attack and Passive");
            hasErrors = chkAutoAttack.ValidateField(() => chkAutoAttack.Checked && chkNoCombatUse.Checked,
                "Cannot be Auto-Attack and No-Combat-Use");
